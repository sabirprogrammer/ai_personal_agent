...
type PhoneStep = "phone" | "method" | "otp";
...
export default function SignIn() {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // Set phone user in auth context
      setPhoneUser({
        id: data.user.id,
        name: data.user.name,
      ...
      <div className="w-full max-w-md glass-premium rounded-3xl p-8 sm:p-10 relative z-10 shadow-2xl shadow-purple-950/20 border border-white/5">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-400 text-xs mt-1.5 text-center">
            Sign in to access your chief-of-staff AI personal assistant.
          </p>
        </div>
        ...
        {authMode === "credentials" && (
          <>
            ...
            <div className="flex items-center my-6">
              ...
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider px-3">or credentials</span>
              <div className="flex-1 h-px bg-white/10"></div>
            </div>
            ...
            <form onSubmit={handleSignIn} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3  border border-white/10 hover:border-white/20 focus:border-purple-500 rounded-xl text-white text-sm focus:outline-none transition"
                />
              </div>
            ...
            {phoneStep === "phone" && (
              <>
                ...
                <button
                  ...
                  className="w-full py-3 px-6 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold rounded-xl text-sm transition duration-200 transform active:scale-[0.98] cursor-pointer flex items-center justify-center text-white"
                >
                  {loading ? (
                    <svg className="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
        ...
        <div className="mt-8 pt-6 border-t border-white/5 text-center text-xs text-slate-400 flex flex-col space-y-2.5">
          <span>
            ...
            <Link href="/sign-up" className="text-purple-400 hover:text-purple-300 font-semibold transition">
              Create an Account
            </Link>
          </span>
